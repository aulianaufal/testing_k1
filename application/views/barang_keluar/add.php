                ...
                <div class="row form-group">
                    ...
                    <div class="col-md-6">
                        <input value="<?= set_value('tanggal_keluar', date('Y-m-d')); ?>" name="tanggal_keluar"
                            id="Tanggal" type="text" class="form-control date" placeholder="Tanggal keluar...">
                        <?= form_error('tanggal_keluar', '<small class="text-danger">', '</small>'); ?>
                    </div>
                </div>

                <div class="row form-group">
                    <label class="col-md-4 text-md-right" for="customer_id">Supplier</label>
                    <div class="col-md-6">
                        <div class="input-group">
                            <select name="customer_id" id="customer_id" class="form-control">
                                <option value="">Pilih customer</option>
                                <?php foreach ($customer as $s): ?>
                                    <option value="<?= $s['id_customer'] ?>" <?= set_select('customer_id', $s['id_customer']); ?>><?= $s['nama_customer'] ?></option>
                                <?php endforeach; ?>
                            </select>
                            <div class="input-group-append">
                                <a class="btn btn-secondary" href="<?= base_url('customer/add'); ?>"><i
...
<script>
    $(document).ready(function () {
        ...
        $('#id_jenis').change(function () {
            ...
            if (id_jenis != '') {
                var url = '<?= base_url("barangkeluar/get_barang_by_jenis/") ?>' + id_jenis;
                $.ajax({
                    url: url,
                    type: 'GET',
                    dataType: 'json',
        ...
        $('#barang_id').change(function () {
            ...
            if (barang_id) {
                var url = '<?= base_url('barangkeluar/get_barang_details/') ?>' + barang_id;
                $.ajax({
                    url: url,
                    type: 'GET',
                    dataType: 'json',
        ...
        function hitungTotal() {
            var jumlah = parseFloat($('#jumlah_keluar').val()) || 0;
            var harga = parseFloat($('#harga_beli').val()) || 0;
            var total = jumlah * harga;
            $('#total').val(total);
        }

        $('#jumlah_keluar, #harga_beli').on('input', function () {
            hitungTotal();
        });
        ...
        $('#jumlah_keluar, #harga_setelah_potongan').on('input', function () {
            hitungTotal();
        });
